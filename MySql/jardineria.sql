...
select distinct estados as estado_de_la_orden from pedido;
...
select distinct c.codigo_cliente,c.nombre_cliente from cliente c inner join pago p on c.codigo_cliente = p.codigo_cliente and year(p.fecha_pago) = 2008;
#9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos que no han sido entregados a tiempo.
select estado, codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido where estado = "pendiente";
...
select estado, codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido where fecha_entrega = adddate(fecha_esperada, interval -2 day);#ADDDATE
select estado, codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido where datediff(fecha_entrega,fecha_esperada) = -2;#DATEDIFF
...
select p.* from producto p,gama_producto gm where p.gama = gm.gama and p.gama ="Ornamentales" and p.cantidad_en_stock = 100;
#16. Devuelve un listado con todos los clientes que sean de la ciudad de Madrid y cuyo representante de ventas tenga el código de empleado 11 o 30.
select c.* from cliente c,empleado e where c.codigo_empleado_rep_ventas = e.codigo_empleado and c.ciudad = "madrid" and c.codigo_empleado_rep_ventas = 11 or c.codigo_empleado_rep_ventas = e.codigo_empleado and c.ciudad = "madrid" and c.codigo_empleado_rep_ventas = 30 order by c.codigo_empleado_rep_ventas asc;

#Consultas multitabla (Composición interna)
#Las consultas se deben resolver con INNER JOIN.

#1. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
select c.nombre_cliente as nombreDelCliente,e.nombre as nombreDelRepresentante, e.apellido1 as apellidoDelRepresentante from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado;
#2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente, e.nombre, p.fecha_pago from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado inner join pago p on c.codigo_cliente = p.codigo_cliente;
#3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente, e.nombre, p.fecha_pago from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado left join pago p on c.codigo_cliente = p.codigo_cliente where p.fecha_pago is null;
#4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select c.codigo_cliente, c.nombre_cliente, p.fecha_pago, e.nombre, o.ciudad from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado inner join pago p on c.codigo_cliente = p.codigo_cliente inner join oficina o on e.codigo_oficina = o.codigo_oficina;
#5. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select c.codigo_cliente, c.nombre_cliente, p.fecha_pago, e.nombre, o.ciudad from cliente c inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado left join pago p on c.codigo_cliente = p.codigo_cliente inner join oficina o on e.codigo_oficina = o.codigo_oficina where p.fecha_pago is null;
